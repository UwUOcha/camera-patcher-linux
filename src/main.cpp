    ...
    bool attach(pid_t target_pid) {
        ...
        if (memFd < 0) {
            ...
            if (errno == EACCES) {
                ...
                std::cerr << "1. Dota 2 runs in a container (Steam Runtime)\n";
                std::cerr << "2. SELinux/AppArmor is blocking access\n";
                std::cerr << "3. Process is in different namespace\n\n";
                std::cerr << "Solutions:\n";
                std::cerr << "1. Run diagnostics: ./diagnose.sh\n";
                std::cerr << "2. Check SELinux: sudo setenforce 0\n";
                std::cerr << "3. Find actual game PID: pgrep -af dota2\n";
                std::cerr << "========================\n";
            }
...
std::vector<pid_t> findProcessesByName(const std::string& processName) {
    ...
    while ((entry = readdir(dir)) != nullptr) {
        ...
        if (cmdline.find("dota2_patcher") != std::string::npos) continue;
...
void printMenu() {
    std::cout << "\n=== Dota 2 Camera Patcher (Direct Memory Access) ===\n";
...
int main() {
    std::cout << "Dota 2 Camera Patcher (Direct /proc/mem access)\n";
    std::cout << "================================================\n\n";

    if (geteuid() != 0) {
        std::cerr << "⚠️  WARNING: Not running as root!\n";
        std::cerr << "This program requires root privileges.\n";
        std::cerr << "Please run: sudo ./dota2_patcher\n\n";
        return 1;
    }

    std::cout << "Searching for dota2 processes...\n";

    auto pids = findProcessesByName("dota2");
    if (pids.empty()) {
        std::cerr << "ERROR: No dota2 process found!\n";
        std::cerr << "Make sure Dota 2 is running.\n";
        return 1;
    }

    std::cout << "Found " << pids.size() << " dota2 process(es):\n";
    ...
    if (pids.size() == 1) {
        ...
        std::cerr << "No valid dota2 process found!\n";
    ...
    if (!mem.attach(dotaPid)) {
        ...
        std::cerr << "\nTry running diagnostics: ./diagnose.sh\n";
        return 1;
    }
